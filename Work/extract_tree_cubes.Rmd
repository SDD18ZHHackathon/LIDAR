...
las_2 <- las_object@data[which(las@data$Classification %in% c(2)),]

plot(las_object)

writeLAS(las_object,paste('tree_las',tree_idx,'.las'))
```
```{bash}
mkdir tree_export
```
...
export_tree <- function(tree_idx){
  ...
  ground_points <- las_object@data[which(las@data$Classification %in% c(2)),]
  ground_estimate = mean(las_2$Z,na.rm=T)
  ...
  las_pruned <- las_object@data[which(las_object@data$Z < ground_estimate+50 & las_object@data$Z > ground_estimate )]
  
  writeLAS(LAS(las_pruned),paste('tree_export/',filtered_baumkataster[tree_idx,'OBJECTID'],'.las',sep='')[1])
...
```{r}
for (i in seq(length(filtered_coors))) {
  export_tree(i)
}
```
